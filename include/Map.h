...
class Map {
  ...
  void serialize(Archive& ar, const unsigned int version) {
    ...
    ar& mnMaxKFid;
    ar& mnBigChangeIdx;
    // Set of KeyFrames and MapPoints, in this version the set serializator is
    // not working
    // ar & mspKeyFrames;
    // ar & mspMapPoints;
 ...
 public:
  ...
  void ApplyScaledRotation(const cv::Mat& R, const float s,
                           const bool bScaledVel = false,
                           const cv::Mat t = cv::Mat::zeros(cv::Size(1, 3),
                                                            CV_32F));
