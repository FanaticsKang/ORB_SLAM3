...
void LoopClosing::Run() {
      ...
      if (NewDetectCommonRegions()) {
        if (mbMergeDetected) {
          if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
            ...
            Verbose::PrintMess(
                "Number of KFs in the current map: " +
                    to_string(mpCurrentKF->GetMap()->KeyFramesInMap()),
                Verbose::VERBOSITY_DEBUG);
            cv::Mat mTmw = mpMergeMatchedKF->GetPose();
            g2o::Sim3 gSmw2(
                Converter::toMatrix3d(mTmw.rowRange(0, 3).colRange(0, 3)),
                Converter::toVector3d(mTmw.rowRange(0, 3).col(3)), 1.0);
            cv::Mat mTcw = mpCurrentKF->GetPose();
            g2o::Sim3 gScw1(
                Converter::toMatrix3d(mTcw.rowRange(0, 3).colRange(0, 3)),
                Converter::toVector3d(mTcw.rowRange(0, 3).col(3)), 1.0);
            ...
            if (mpCurrentKF->GetMap()->IsInertial() &&
              ...
              if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
                   mpTracker->mSensor == System::IMU_STEREO) &&
                  mpCurrentKF->GetMap()
                      ->GetIniertialBA1())  // TODO, maybe with GetIniertialBA1
              {
                Eigen::Vector3d phi =
            ...
            //                        cout << "tw2w1: " <<
            //                        mSold_new.translation() << endl; cout <<
            //                        "Rw2w1: " <<
            //                        mSold_new.rotation().toRotationMatrix() <<
            //                        endl; cout << "Angle Rw2w1: " <<
            //                        180*LogSO3(mSold_new.rotation().toRotationMatrix())/3.14
            //                        << endl; cout << "scale w2w1: " <<
            //                        mSold_new.scale() << endl;
        ...
        if (mbLoopDetected) {
          ...
          if (mpCurrentKF->GetMap()->IsInertial()) {
            ...
            g2o::Sim3 g2oTwc(
                Converter::toMatrix3d(Twc.rowRange(0, 3).colRange(0, 3)),
            ...
            // cout << "Rw2w1: " << g2oSww_new.rotation().toRotationMatrix() <<
            // endl; cout << "Angle Rw2w1: " << 180*phi/3.14 << endl; cout <<
            // "scale w2w1: " << g2oSww_new.scale() << endl;
            ...
            if (fabs(phi(0)) < 0.008f && fabs(phi(1)) < 0.008f &&
              ...
              mvpLoopMapPoints =
                  mvpLoopMPs;  //*mvvpLoopMapPoints[nCurrentIndex];
              CorrectLoop();
            } else {
              cout << "BAD LOOP!!!" << endl;
            }
  ...
  // ofstream f_stats;
  // f_stats.open("PlaceRecognition_stats" + mpLocalMapper->strSequence +
  // ".txt"); f_stats << "# current_timestamp, matched_timestamp, [0:Loop,
  // 1:Merge]" << endl; f_stats << fixed; for(int i=0; i<
  // vdPR_CurrentTime.size(); ++i)
  //{
  //    f_stats  << 1e9*vdPR_CurrentTime[i] << "," << 1e9*vdPR_MatchedTime[i] <<
  //    "," << vnPR_TypeRecogn[i] << endl;
  //}
...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  if (mnLoopNumCoincidences > 0) {
    ...
    g2o::Sim3 gScl(Converter::toMatrix3d(mTcl.rowRange(0, 3).colRange(0, 3)),
                   Converter::toVector3d(mTcl.rowRange(0, 3).col(3)), 1.0);
    ...
    if (bCommonRegion) {
      ...
      if (!mbLoopDetected) {
        ...
        // f_succes_pr << "% Number of spatial consensous: " <<
        // std::to_string(mnLoopNumCoincidences) << endl;
  ...
  // cout << "LC: bMergeDetectedInKF: " << bMergeDetectedInKF << "
  // bLoopDetectedInKF: " << bLoopDetectedInKF << endl;
  ...
  /*#ifdef COMPILEDWITHC11
      std::chrono::steady_clock::time_point timeStartGeoBoW =
  std::chrono::steady_clock::now(); #else
      std::chrono::monotonic_clock::time_point timeStartGeoBoW =
  std::chrono::monotonic_clock::now(); #endif*/
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
  ...
  cout << "REFFINE-SIM3: Projection from last KF with " << nNumProjMatches
       << " matches" << endl;
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    g2o::Sim3 gSwm(Converter::toMatrix3d(mTwm.rowRange(0, 3).colRange(0, 3)),
                   Converter::toVector3d(mTwm.rowRange(0, 3).col(3)), 1.0);
    ...
    cout << "REFFINE-SIM3: Optimize Sim3 from last KF with " << numOptMatches
         << " inliers" << endl;

    if (numOptMatches > nProjOptMatches) {
      g2o::Sim3 gScw_estimation(
          Converter::toMatrix3d(mScw.rowRange(0, 3).colRange(0, 3)),
      ...
      // cout << "REFFINE-SIM3: Projection with optimize Sim3 from last KF with
      // " << nNumProjMatches << " matches" << endl;
...
bool LoopClosing::DetectCommonRegionsFromBoW(
  ...
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    for (size_t j = 0; j < vpCovKFi.size(); ++j) {
      ...
      // cout << "BoW: " << num << " putative matches with KF " <<
      // vpCovKFi[j]->mnId << endl;
    ...
    // cout <<"BoW: " << numBoWMatches << " independent putative matches" <<
    // endl;
    if (!bAbortByNearKF &&
        numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      /*cout << "-------------------------------" << endl;
      cout << "Geometric validation with " << numBoWMatches << endl;
      cout << "KFc: " << mpCurrentKF->mnId << "; KFm: " <<
      pMostBoWMatchesKF->mnId << endl;*/
      ...
      if (bConverge) {
        ...
        g2o::Sim3 gSmw(
            Converter::toMatrix3d(pMostBoWMatchesKF->GetRotation()),
            Converter::toVector3d(pMostBoWMatchesKF->GetTranslation()), 1.0);
        g2o::Sim3 gScw =
            gScm *
        ...
        if (numProjMatches >= nProjMatches) {
          ...
          // cout <<"BoW: " << numOptMatches << " inliers in the Sim3
          // optimization" << endl; cout << "Inliers in Sim3 optimization: " <<
          // numOptMatches << endl;

          if (numOptMatches >= nSim3Inliers) {
            // cout <<"BoW: " << numOptMatches << " inliers in Sim3
            // optimization" << endl;
            g2o::Sim3 gSmw(
                Converter::toMatrix3d(pMostBoWMatchesKF->GetRotation()),
            ...
            // cout <<"BoW: " << numProjOptMatches << " matches after of the
            // Sim3 optimization" << endl;

            if (numProjOptMatches >= nProjOptMatches) {
              cout << "BoW: Current KF " << mpCurrentKF->mnId
                   << "; candidate KF " << pKFi->mnId << endl;
              cout << "BoW: There are " << numProjOptMatches
                   << " matches between them with the optimized Sim3" << endl;
              ...
              // cout << "BoW: Coord in X -> " << min_x << ", " << max_x << ";
              // and Y -> " << min_y << ", " << max_y << endl; cout << "BoW:
              // features area in X -> " << (max_x - min_x) << " and Y -> " <<
              // (max_y - min_y) << endl;
              ...
              // for(int j=0; j<vpCurrentCovKFs.size(); ++j)
              {
                ...
                g2o::Sim3 gSjc(
                    Converter::toMatrix3d(mTjc.rowRange(0, 3).colRange(0, 3)),
                    Converter::toVector3d(mTjc.rowRange(0, 3).col(3)), 1.0);
                ...
                if (bValid) {
                  // cout << "BoW: KF " << pKFj->mnId << " has " <<
                  // numProjMatches_j << " matches" << endl;
                  ...
                  cout << "BoW: KF " << pKFi->mnId << " to KF " << pKFj->mnId
                       << " is separated by " << dist << " meters" << endl;
                  ...
                  cout << "BoW: Rotation in angles (x, y, z) -> (" << v_euler[0]
                       << ", " << v_euler[1] << ", " << v_euler[2] << ")"
                       << endl;
                  nNumKFs++;
  ...
  if (nBestMatchesReproj > 0) {
    ...
    //        f_succes_pr << mpCurrentKF->mNameFile << " " <<
    //        std::to_string(maxStage) << endl; f_succes_pr << "% NumCand: " <<
    //        std::to_string(numCandidates) << "; matches: " <<
    //        std::to_string(maxMatched) << endl;
  }
  return false;
}
...
bool LoopClosing::DetectCommonRegionsFromLastKF(
  ...
  nNumProjMatches = FindMatchesByProjection(
      pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
  // cout << "Projection from last KF with " << nNumProjMatches << " matches" <<
  // endl;

  int nProjMatches = 30;
  if (nNumProjMatches >= nProjMatches) {
    /*cout << "PR_From_LastKF: KF " << pCurrentKF->mnId << " has " <<
    nNumProjMatches << " with KF " << pMatchedKF->mnId << endl;
    ...
    // cout << "PR_From_LastKF: Coord in X -> " << min_x << ", " << max_x << ";
    // and Y -> " << min_y << ", " << max_y << endl; cout << "PR_From_LastKF:
    // features area in X -> " << (max_x - min_x) << " and Y -> " << (max_y -
    // min_y) << endl;
...
void LoopClosing::CorrectLoop() {
...

  {
    ...
    cout << "LC: there are " << CorrectedSim3.size()
         << " KFs in the local window" << endl;
    for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
      ...
      if (bImuInit) {
        Eigen::Matrix3d Rcor =
            eigR.transpose() * g2oSiw.rotation().toRotationMatrix();
...
void LoopClosing::MergeLocal() {
  Verbose::PrintMess("MERGE-VISUAL: Merge Visual detected!!!!",
                     Verbose::VERBOSITY_NORMAL);
  ...
  Verbose::PrintMess("MERGE-VISUAL: Local Map stopped",
                     Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Initially there are " +
          to_string(pCurrentMap->KeyFramesInMap()) + " KFs and " +
          to_string(pCurrentMap->MapPointsInMap()) + " MPs in the active map ",
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("MERGE-VISUAL: Initially there are " +
                         to_string(pMergeMap->KeyFramesInMap()) + " KFs and " +
                         to_string(pMergeMap->MapPointsInMap()) +
                         " MPs in the matched map ",
                     Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Initial number of KFs in local window from active map: " +
          to_string(spLocalWindowKFs.size()),
      Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Last number of KFs in local window from the active map: " +
          to_string(spLocalWindowKFs.size()),
      Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Number of MPs in local window from active map: " +
          to_string(spLocalWindowMPs.size()),
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("MERGE-VISUAL: Number of MPs in the active map: " +
                         to_string(pCurrentMap->GetAllMapPoints().size()),
                     Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Initial number of KFs in the local window from matched "
      "map: " +
          to_string(spMergeConnectedKFs.size()),
      Verbose::VERBOSITY_DEBUG);
  nNumTries = 0;
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Last number of KFs in the localwindow from matched map: " +
          to_string(spMergeConnectedKFs.size()),
      Verbose::VERBOSITY_DEBUG);
  ...
  g2o::Sim3 g2oNonCorrectedSwc(Converter::toMatrix3d(Rwc),
                               Converter::toVector3d(twc), 1.0);
  ...
  // TODO Time test
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeStartTransfMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeStartTransfMerge =
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs) {
    ...
    // cout << "angle: " <<
    // 180*LogSO3(mg2oMergeScw.rotation().toRotationMatrix())/3.14 << endl; cout
    // << "t: " << mg2oMergeScw.translation() << endl;
    ...
    // cout << "After -> Map current: " << pCurrentMap << "; New map: " <<
    // pKFi->GetMap() << endl;

    if (pCurrentMap->isImuInitialized()) {
      Eigen::Matrix3d Rcor =
          eigR.transpose() *
          vNonCorrectedSim3[pKFi].rotation().toRotationMatrix();
      pKFi->mVwbMerge = Converter::toCvMat(Rcor) * pKFi->GetVelocity();
      // pKFi->SetVelocity(Converter::toCvMat(Rcor)*pKFi->GetVelocity()); //
      // TODO: should add here scale s
    }
  ...
  for (MapPoint* pMPi : spLocalWindowMPs) {
    ...
    Eigen::Matrix<double, 3, 1> eigP3Dw = Converter::toVector3d(P3Dw);
    Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw =
        g2oCorrectedSwi.map(g2oNonCorrectedSiw.map(eigP3Dw));
    Eigen::Matrix3d eigR = g2oCorrectedSwi.rotation().toRotationMatrix();
    Eigen::Matrix3d Rcor =
        eigR * g2oNonCorrectedSiw.rotation().toRotationMatrix();
...
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeFinishTransfMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeFinishTransfMerge =
      std::chrono::monotonic_clock::now();
#endif
  std::chrono::duration<double, std::milli> timeTransfMerge =
      timeFinishTransfMerge - timeStartTransfMerge;  // Time in milliseconds
  Verbose::PrintMess(
      "MERGE-VISUAL: TRANSF ms: " + to_string(timeTransfMerge.count()),
      Verbose::VERBOSITY_DEBUG);

  // TODO Time test
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeStartCritMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeStartCritMerge =
      std::chrono::monotonic_clock::now();
#endif
  {
...
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeFinishCritMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeFinishCritMerge =
      std::chrono::monotonic_clock::now();
#endif
  std::chrono::duration<double, std::milli> timeCritMerge =
      timeFinishCritMerge - timeStartCritMerge;  // Time in milliseconds
  Verbose::PrintMess(
      "MERGE-VISUAL: New current map: " + to_string(pMergeMap->GetId()),
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess(
      "MERGE-VISUAL: CRITICAL ms: " + to_string(timeCritMerge.count()),
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("MERGE-VISUAL: LOCAL MAPPING number of KFs: " +
                         to_string(mpLocalMapper->KeyframesInQueue()),
                     Verbose::VERBOSITY_DEBUG);
  ...
  while(pNewChild /*&& spLocalWindowKFs.find(pNewChild) != spLocalWindowKFs.end()*/)
    ...
    // cout << "The new parent of KF " << pNewChild->mnId << " was " <<
    // pNewChild->GetParent()->mnId << endl;
  ...
  // std::copy(spMapPointCurrent.begin(), spMapPointCurrent.end(),
  // std::back_inserter(vpCheckFuseMapPoint));
  ...
  // TODO Time test
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeStartFuseMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeStartFuseMerge =
      std::chrono::monotonic_clock::now();
#endif
...
#ifdef COMPILEDWITHC11
  std::chrono::steady_clock::time_point timeFinishFuseMerge =
      std::chrono::steady_clock::now();
#else
  std::chrono::monotonic_clock::time_point timeFinishFuseMerge =
      std::chrono::monotonic_clock::now();
#endif
  std::chrono::duration<double, std::milli> timeFuseMerge =
      timeFinishFuseMerge - timeStartFuseMerge;  // Time in milliseconds
  Verbose::PrintMess(
      "MERGE-VISUAL: FUSE DUPLICATED ms: " + to_string(timeFuseMerge.count()),
      Verbose::VERBOSITY_DEBUG);

  // Update connectivity
  Verbose::PrintMess(
      "MERGE-VISUAL: Init to update connections in the welding area",
      Verbose::VERBOSITY_DEBUG);
  for (KeyFrame* pKFi : spLocalWindowKFs) {
    if (!pKFi || pKFi->isBad()) continue;
  ...
  Verbose::PrintMess(
      "MERGE-VISUAL: Finish to update connections in the welding area",
      Verbose::VERBOSITY_DEBUG);
  ...
  if (mpTracker->mSensor == System::IMU_MONOCULAR ||
      mpTracker->mSensor == System::IMU_STEREO) {
    Verbose::PrintMess("MERGE-VISUAL: Visual-Inertial",
                       Verbose::VERBOSITY_DEBUG);
    Optimizer::MergeInertialBA(mpLocalMapper->GetCurrKF(), mpMergeMatchedKF,
                               &bStop, mpCurrentKF->GetMap(), vCorrectedSim3);
  } else {
    Verbose::PrintMess("MERGE-VISUAL: Visual", Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess("MERGE-VISUAL: Local current window->" +
                           to_string(vpLocalCurrentWindowKFs.size()) +
                           "; Local merge window->" +
                           to_string(vpMergeConnectedKFs.size()),
                       Verbose::VERBOSITY_DEBUG);
  ...
  if (vpCurrentMapKFs.size() == 0) {
    Verbose::PrintMess(
        "MERGE-VISUAL: There are not KFs outside of the welding area",
        Verbose::VERBOSITY_DEBUG);
  } else {
    Verbose::PrintMess(
        "MERGE-VISUAL: Calculate the new position of the elements outside of "
        "the window",
        Verbose::VERBOSITY_DEBUG);
    // Apply the transformation
    {
      if (mpTracker->mSensor == System::MONOCULAR) {
        ...
        for (KeyFrame* pKFi : vpCurrentMapKFs) {
          ...
          cv::Mat Ric = Tic.rowRange(0, 3).colRange(0, 3);
          cv::Mat tic = Tic.rowRange(0, 3).col(3);
          g2o::Sim3 g2oSim(Converter::toMatrix3d(Ric),
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Apply transformation to all elements of the old map",
        Verbose::VERBOSITY_DEBUG);
...

    {
      ...
      Verbose::PrintMess("MERGE-VISUAL: There are " +
                             to_string(pMergeMap->KeyFramesInMap()) +
                             " KFs in the map",
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("MERGE-VISUAL: It will be inserted " +
                             to_string(vpCurrentMapKFs.size()) +
                             " KFs in the map",
                         Verbose::VERBOSITY_DEBUG);
      ...
      Verbose::PrintMess("MERGE-VISUAL: There are " +
                         ...
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("MERGE-VISUAL: It will be inserted " +
                             to_string(vpCurrentMapMPs.size()) +
                             " MPs in the map",
                         Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("MERGE-VISUAL: Optimaze the essential graph",
                       Verbose::VERBOSITY_DEBUG);
  }

  mpLocalMapper->Release();

  Verbose::PrintMess("MERGE-VISUAL: Finally there are " +
                         to_string(pMergeMap->KeyFramesInMap()) + "KFs and " +
                         to_string(pMergeMap->MapPointsInMap()) +
                         " MPs in the complete map ",
                     Verbose::VERBOSITY_DEBUG);
...
void LoopClosing::printReprojectionError(set<KeyFrame*>& spLocalWindowKFs,
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs) {
    ...
    for (size_t j = 0; j < vpMPs.size(); ++j) {
      ...
      if (bIsInImage) {
        ...
        cv::circle(img_i, point_img.pt, 1 /*point_img.octave*/,
                   cv::Scalar(0, 255, 0));
        cv::line(img_i, point_img.pt, reproj_p, cv::Scalar(0, 0, 255));
        num_points++;
      } else {
        // cout << "Reproj out of the image" << endl;
        cv::circle(img_i, point_img.pt, point_img.octave,
                   cv::Scalar(0, 0, 255));
      }
    }
    // cout << "Image painted" << endl;
    string filename_img = path_imgs + "KF" + to_string(mpCurrentKF->mnId) +
                          "_" + to_string(pKFi->mnId) + name + "points" +
                          to_string(num_points) + ".png";
...
void LoopClosing::MergeLocal2() {
...

  {
    ...
    cout << "KFs before empty: " << mpAtlas->GetCurrentMap()->KeyFramesInMap()
         << endl;
    mpLocalMapper->EmptyQueue();
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose& CorrectedPosesMap,
  ...
  cout << "FUSE: Intially there are " << CorrectedPosesMap.size() << " KFs"
       << endl;
...
void LoopClosing::SearchAndFuse(const vector<KeyFrame*>& vConectedKFs,
  ...
  cout << "FUSE-POSE: Initially there are " << vpMapPoints.size() << " MPs"
       << endl;
  cout << "FUSE-POSE: Intially there are " << vConectedKFs.size() << " KFs"
       << endl;
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        for (set<KeyFrame*>::const_iterator sit = sChilds.begin();
          ...
          if (pChild->mnBAGlobalForKF != nLoopKF) {
            ...
            cv::Mat Rcor = pChild->mTcwGBA.rowRange(0, 3).colRange(0, 3).t() *
                           pChild->GetRotation();
