  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (nLoopKF == pMap->GetOriginKF()->mnId) {
      ...
      if (dist > 1) {
        ...
        Verbose::PrintMess("GBA: KF " + to_string(pKF->mnId) +
                               " had been moved " + to_string(dist) + " meters",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess(
            "--Number of observations: " + to_string(numMonoOptPoints) +
                " in mono and " + to_string(numStereoOptPoints) + " in stereo",
            Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--Number of discarded observations: " +
                               to_string(numMonoBadPoints) + " in mono and " +
                               to_string(numStereoBadPoints) + " in stereo",
                           Verbose::VERBOSITY_DEBUG);
      }
    }
  }
...
void Optimizer::FullInertialBA(Map* pMap, int its, const bool bFixLocal,
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (pKFi->bImu && pKFi->mPrevKF->bImu) {
        ...
        if (!bInit) {
          ...
          for (int r = 0; r < 3; r++)
            for (int c = 0; c < 3; c++) InfoG(r, c) = cvInfoG.at<float>(r, c);
          ...
          ear->setVertex(1, VA2);
          cv::Mat cvInfoA =
              pKFi->mpImuPreintegrated->C.rowRange(12, 15).colRange(12, 15).inv(
                  cv::DECOMP_SVD);
          Eigen::Matrix3d InfoA;
          for (int r = 0; r < 3; r++)
            for (int c = 0; c < 3; c++) InfoA(r, c) = cvInfoA.at<float>(r, c);
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (nLoopId == 0) {
      ...
      Converter::toCvMat(VP->estimate().Rcw[0])
          .copyTo(pKFi->mTcwGBA.rowRange(0, 3).colRange(0, 3));
      Converter::toCvMat(VP->estimate().tcw[0])
          .copyTo(pKFi->mTcwGBA.rowRange(0, 3).col(3));
...
int Optimizer::PoseOptimization(Frame* pFrame) {
  ...
  // cout << "PO: vnIndexEdgeMono.size() = " << vnIndexEdgeMono.size() << "
  // vnIndexEdgeRight.size() = " << vnIndexEdgeRight.size() << endl;
  ...
  // cout << "[PoseOptimization]: initial correspondences-> " <<
  // nInitialCorrespondences << " --- outliers-> " << nBad << endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  if (num_fixedKF < 2) {
    // Verbose::PrintMess("LM-LBA: New Fixed KFs had been set",
    // Verbose::VERBOSITY_NORMAL);
    // TODO We set 2 KFs to fixed to avoid a degree of freedom in scale
    list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin();
    unsigned long lowerId = pKF->mnId;
    KeyFrame* pLowerKf;
    unsigned long secondLowerId = pKF->mnId;
  ...
  // Verbose::PrintMess("LM-LBA: There are " + to_string(lLocalKeyFrames.size())
  // + " KFs and " + to_string(lLocalMapPoints.size()) + " MPs to optimize. " +
  // to_string(num_fixedKF) + " KFs are fixed", Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "LM-LBA: opt/fixed KFs: " + to_string(lLocalKeyFrames.size()) + "/" +
          to_string(lFixedCameras.size()),
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("LM-LBA: local MPs: " + to_string(lLocalMapPoints.size()),
                     Verbose::VERBOSITY_DEBUG);
      ...
      if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
        ...
        if (cam0Index != -1 && pKFi->mvuRight[cam0Index] < 0) {
          ...
          nEdges++;
                   ...
                   pKFi->mvuRight[cam0Index] >=
                       0)  // Stereo observation (with rectified images)
        {
          const cv::KeyPoint& kpUn = pKFi->mvKeysUn[cam0Index];
          Eigen::Matrix<double, 3, 1> obs;
  ...
  // << "[ms]" << std::endl; std::cout << "Keyframes: " << nKFs << " ---
  // MapPoints: " << nPoints << " --- Edges: " << nEdges << endl;
  ...
  if (bDoMore) {
    ...
    Verbose::PrintMess("LM-LBA: First optimization has " +
                           to_string(nMonoBadObs) + " monocular and " +
                           to_string(nStereoBadObs) +
                           " stereo bad observations",
                       Verbose::VERBOSITY_DEBUG);

    // Optimize again without the outliers
    // Verbose::PrintMess("LM-LBA: second optimization",
    // Verbose::VERBOSITY_DEBUG); optimizer.initializeOptimization(0);
  ...
  Verbose::PrintMess(
      "LM-LBA: outlier observations: " + to_string(vToErase.size()),
      Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess(
      "LM-LBA: total of observations: " +
          to_string(vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size()),
      Verbose::VERBOSITY_DEBUG);
  ...
  if (!vToErase.empty()) {
    // cout << "LM-LBA: There are " << vToErase.size() << " observations whose
    // will be deleted from the map" << endl;
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    // cout << "LM-LBA: KF " << pKFi->mnId << " had performed " <<
    // pKFi->mnNumberOfOpt << " iterations" << endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  g2o::OptimizationAlgorithmLevenberg* solver =
      new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
  if (pMap->IsInertial()) solver->setUserLambdaInit(100.0);  // TODO uncomment
  // cout << "LM-LBA: lambda init: " << solver->userLambdaInit() << endl;
  ...
  // Verbose::PrintMess("LM-LBA: KFs to optimize added",
  // Verbose::VERBOSITY_DEBUG);
  ...
  // Verbose::PrintMess("LM-LBA: total observations: " +
  // to_string(vpMapPointEdgeMono.size()+vpMapPointEdgeStereo.size()),
  // Verbose::VERBOSITY_DEBUG);
  ...
  if (bDoMore) {
    ...
    // Verbose::PrintMess("LM-LBA: First optimization has " +
    // to_string(nMonoBadObs) + " monocular and " + to_string(nStereoBadObs) + "
  ...
  // Verbose::PrintMess("LM-LBA: outlier observations: " +
  // to_string(vToErase.size()), Verbose::VERBOSITY_DEBUG);
  bool bRedrawError = false;
  if (vToErase.size() >=
      (vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size()) * 0.5) {
    Verbose::PrintMess(
        "LM-LBA: ERROR IN THE OPTIMIZATION, MOST OF THE POINTS HAS BECOME "
        "OUTLIERS",
        Verbose::VERBOSITY_NORMAL);
  ...
  if (!vToErase.empty()) {
    ...
    if (bRedrawError) {
      ...
      f_lba << "# KF id, Initial Num CovKFs, Final Num CovKFs, Initial Num "
               "MPs, Fimal Num MPs"
            << endl;
      f_lba << fixed;
      ...
      for (KeyFrame* pKFi : lLocalKeyFrames) {
        ...
        f_lba << pKFi->mnId << ", " << mspInitialConnectedKFs[pKFi] << ", "
              << finalNumberCovKFs << ", " << mspInitialObservationKFs[pKFi]
              << ", " << finalNumberMPs << endl;
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    if (dist > 1.0) {
      // bShowStats = true;
      Verbose::PrintMess("LM-LBA: Too much distance in KF " +
                             to_string(pKFi->mnId) + ", " + to_string(dist) +
                             " meters. Current KF " + to_string(pKF->mnId),
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("LM-LBA: Number of connections between the KFs " +
                             to_string(pKF->GetWeight((pKFi))),
                         Verbose::VERBOSITY_DEBUG);
      ...
      for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++) {
        if (vpEdgeKFStereo[i] != pKFi) continue;
        g2o::EdgeStereoSE3ProjectXYZ* e = vpEdgesStereo[i];
        MapPoint* pMP = vpMapPointEdgeStereo[i];
      ...
      Verbose::PrintMess("LM-LBA: Good observations in mono " +
                             to_string(numMonoMP) + " and stereo " +
                             to_string(numStereoMP),
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("LM-LBA: Bad observations in mono " +
                             to_string(numBadMonoMP) + " and stereo " +
                             to_string(numBadStereoMP),
                         Verbose::VERBOSITY_DEBUG);
    }
  }
...
void Optimizer::OptimizeEssentialGraph(
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    vZvectors[nIDi] =
        vScw[nIDi].rotation().toRotationMatrix() * z_vec;  // For debugging
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    /*if(count_kf<3)
        cout << "EG: kf with " << count_kf << " edges!!    ID: " << pKF->mnId <<
       endl;*/
  }
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    pKFi->SetPose(Tiw);
    // cout << "angle KF " << nIDi << ": " <<
    // (180.0/3.1415)*acos(vZvectors[nIDi].dot(eigR*z_vec)) << endl;
  }
...
void Optimizer::OptimizeEssentialGraph6DoF(
    KeyFrame* pCurKF, vector<KeyFrame*>& vpFixedKFs,
    vector<KeyFrame*>& vpFixedCorrectedKFs, vector<KeyFrame*>& vpNonFixedKFs,
    vector<MapPoint*>& vpNonCorrectedMPs, double scale) {
  ...
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>> vScw(nMaxKFid +
                                                                    1);
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>> vScw_bef(
      nMaxKFid + 1);
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>> vCorrectedSwc(
      nMaxKFid + 1);
  ...
  for (KeyFrame* pKFi : vpFixedCorrectedKFs) {
    ...
    Eigen::Matrix<double, 3, 3> Rcw_bef =
        Converter::toMatrix3d(Tcw_bef.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1> tcw_bef =
        Converter::toVector3d(Tcw_bef.rowRange(0, 3).col(3)) / scale;
  ...
  Verbose::PrintMess("Opt_Essential: vpFixedCorrectedKFs loaded",
                     Verbose::VERBOSITY_DEBUG);
  ...
  for (KeyFrame* pKFi : vpNonFixedKFs) {
    ...
    // Eigen::Matrix<double,3,3> Rcw =
    // Converter::toMatrix3d(Tcw.rowRange(0,3).colRange(0,3));
    // Eigen::Matrix<double,3,1> tcw =
    // Converter::toVector3d(Tcw.rowRange(0,3).col(3));
    Eigen::Matrix<double, 3, 3> Rcw =
        Converter::toMatrix3d(pKFi->GetRotation());
    Eigen::Matrix<double, 3, 1> tcw =
        Converter::toVector3d(pKFi->GetTranslation()) / scale;
    g2o::SE3Quat Siw(Rcw, tcw);
    vScw_bef[nIDi] = Siw;
    VSE3->setEstimate(Siw);
  ...
  Verbose::PrintMess("Opt_Essential: vpNonFixedKFs loaded",
                     Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess("Opt_Essential: List of KF loaded",
                     Verbose::VERBOSITY_DEBUG);

  const Eigen::Matrix<double, 6, 6> matLambda =
  ...
  for (KeyFrame* pKFi : vpKFs) {
    ...
    else
    {
        ...
        Swi = g2o::Sim3(Converter::toMatrix3d(Twi.rowRange(0, 3).colRange(0,
    3)), Converter::toVector3d(Twi.rowRange(0, 3).col(3)),1.0);
    }*/
    ...
    if (pParentKFi && spKFs.find(pParentKFi) != spKFs.end()) {
      ...
      else
      {
          ...
          Sjw = g2o::Sim3(Converter::toMatrix3d(Tjw.rowRange(0, 3).colRange(0,
      3)), Converter::toVector3d(Tjw.rowRange(0, 3).col(3)),1.0);
      }*/
    ...
    for (set<KeyFrame*>::const_iterator sit = sLoopEdges.begin(),
      ...
      if (spKFs.find(pLKF) != spKFs.end() && pLKF->mnId < pKFi->mnId) {
        ...
        else
        {
            ...
            Slw = g2o::Sim3(Converter::toMatrix3d(Tlw.rowRange(0, 3).colRange(0,
        3)), Converter::toVector3d(Tlw.rowRange(0, 3).col(3)),1.0);
        }*/
      ...
      if (pKFn && pKFn != pParentKFi && !pKFi->hasChild(pKFn) &&
        ...
        if (!pKFn->isBad() && pKFn->mnId < pKFi->mnId) {
          ...
          else
          {
              ...
              Snw = g2o::Sim3(Converter::toMatrix3d(Tnw.rowRange(0,
          3).colRange(0, 3)), Converter::toVector3d(Tnw.rowRange(0,
          3).col(3)),1.0);
          }*/
  ...
  Verbose::PrintMess("Opt_Essential: Finish the optimization",
                     Verbose::VERBOSITY_DEBUG);

  unique_lock<mutex> lock(pMap->mMutexMapUpdate);

  Verbose::PrintMess("Opt_Essential: Apply the new pose to the KFs",
                     Verbose::VERBOSITY_DEBUG);
  // SE3 Pose Recovering. Sim3:[sR t;0 1] -> SE3:[R t/s;0 1]
  for (KeyFrame* pKFi : vpNonFixedKFs) {
    if (pKFi->isBad()) continue;
    ...
    g2o::VertexSE3Expmap* VSE3 =
        static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(nIDi));
    ...
    /*{
        ...
        if(dist > 1.0)
        {
            ...
            cout << "--To much distance correction in EssentGraph: It has
    connected " << pKFi->GetVectorCovisibleKeyFrames().size() << " KFs" << endl;
        }
        ...
        for(int j=0; j<vpMapPointsKFi.size(); ++j)
        {
            ...
            cv::putText(imLeft, strNumOBs, pKFi->mvKeys[j].pt,
    CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
        }

        string namefile = "./test_OptEssent/Essent_" + to_string(pCurKF->mnId) +
    "_KF" + to_string(pKFi->mnId) +"_D" + to_string(dist) +".png";
  ...
  Verbose::PrintMess("Opt_Essential: Apply the new pose to the MPs",
                     Verbose::VERBOSITY_DEBUG);
  cout << "Opt_Essential: number of points -> " << vpNonCorrectedMPs.size()
       << endl;
  // Correct points. Transform to "non-optimized" reference keyframe pose and
  // transform back with optimized pose
  for (MapPoint* pMPi : vpNonCorrectedMPs) {
    ...
    // Verbose::PrintMess("Opt_Essential: MP id " + to_string(pMPi->mnId),
    // Verbose::VERBOSITY_DEBUG);
    /*int nIDr;
    ...
    // Eigen::Matrix<double,3,3> RNonCorrectedwr =
    // Converter::toMatrix3d(TNonCorrectedwr.rowRange(0,3).colRange(0,3));
    // Eigen::Matrix<double,3,1> tNonCorrectedwr =
    // Converter::toVector3d(TNonCorrectedwr.rowRange(0,3).col(3));
    Srw = vScw_bef
        [pRefKF
             ->mnId];  // g2o::SE3Quat(RNonCorrectedwr,tNonCorrectedwr).inverse();

    cv::Mat Twr = pRefKF->GetPoseInverse();
    Eigen::Matrix<double, 3, 3> Rwr =
        Converter::toMatrix3d(Twr.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1> twr =
        Converter::toVector3d(Twr.rowRange(0, 3).col(3));
    ...
    cv::Mat cvCorrectedP3Dw = Converter::toCvMat(eigCorrectedP3Dw);
    // cout << "Opt_Essential: Converted the position to the OpenCV format" <<
    // endl;
    pMPi->SetWorldPos(cvCorrectedP3Dw);
    // cout << "Opt_Essential: Loaded the corrected position in the MP object"
    // << endl;

    pMPi->UpdateNormalAndDepth();
  }

  Verbose::PrintMess("Opt_Essential: End of the optimization",
                     Verbose::VERBOSITY_DEBUG);
}
...
void Optimizer::OptimizeEssentialGraph(KeyFrame* pCurKF,
  ...
  for (KeyFrame* pKFi : vpFixedCorrectedKFs) {
    ...
    Eigen::Matrix<double, 3, 3> Rcw_bef =
        Converter::toMatrix3d(Tcw_bef.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1> tcw_bef =
        Converter::toVector3d(Tcw_bef.rowRange(0, 3).col(3));
...
void Optimizer::OptimizeEssentialGraph(
    KeyFrame* pCurKF, const LoopClosing::KeyFrameAndPose& NonCorrectedSim3,
    const LoopClosing::KeyFrameAndPose& CorrectedSim3) {
  // Setup optimizer
  Map* pMap = pCurKF->GetMap();
  g2o::SparseOptimizer optimizer;
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    if (pKF->mnBALocalForKF == pCurKF->mnId ||
        pKF->mnBAFixedForKF == pCurKF->mnId) {
  ...
  Verbose::PrintMess("edges pose graph: " + to_string(count_edges[0]) + ", " +
                         to_string(count_edges[1]) + ", " +
                         to_string(count_edges[2]),
                     Verbose::VERBOSITY_DEBUG);
  // Optimize!
  optimizer.initializeOptimization();
  optimizer.setVerbose(false);
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
  ...
  for (int i = 0; i < N; i++) {
    ...
    const int i2 = get<0>(pMP2->GetIndexInKeyFrame(pKF2));
    /*if(i2 < 0)
        cout << "Sim3-OPT: Error, there is a matched which is not find it" <<
       endl;*/
  ...
  Verbose::PrintMess(
      "Sim3: There are " + to_string(nCorrespondences) + " matches, " +
          to_string(nInKF2) + " are in the KF and " + to_string(nOutKF2) +
          " are in the connected KFs. There are " + to_string(nMatchWithoutMP) +
          " matches which have not an associate MP",
      Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
      "Sim3: First Opt -> Correspondences: " + to_string(nCorrespondences) +
          "; nBad: " + to_string(nBad) +
          "; nBadOutKF2: " + to_string(nBadOutKF2),
      Verbose::VERBOSITY_DEBUG);
  ...
  // Recover optimized Sim3
  // Verbose::PrintMess("Sim3: Initial seed " + g2oS12,
  // Verbose::VERBOSITY_DEBUG);
  g2o::VertexSim3Expmap* vSim3_recov =
      static_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(0));
  g2oS12 = vSim3_recov->estimate();
  // Verbose::PrintMess("Sim3: Optimized solution " + g2oS12,
  // Verbose::VERBOSITY_DEBUG);
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
                            ...
                            vector<KeyFrame*>& vpMatches1KF, g2o::Sim3& g2oS12,
                            const float th2, const bool bFixScale,
                            Eigen::Matrix<double, 7, 7>& mAcumHessian,
                            const bool bAllPoints) {
  g2o::SparseOptimizer optimizer;
  ...
  const cv::Mat R1w = pKF1->GetRotation();
  const cv::Mat t1w = pKF1->GetTranslation();
  Verbose::PrintMess("Extracted rotation and traslation from the first KF ",
                     Verbose::VERBOSITY_DEBUG);
  const cv::Mat R2w = pKF2->GetRotation();
  const cv::Mat t2w = pKF2->GetTranslation();
  Verbose::PrintMess("Extracted rotation and traslation from the second KF ",
                     Verbose::VERBOSITY_DEBUG);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (i2 < 0)
      Verbose::PrintMess(
          "Sim3-OPT: Error, there is a matched which is not find it",
          Verbose::VERBOSITY_DEBUG);
  ...
  // cout << "Sim3 -> Correspondences: " << nCorrespondences << "; nBad: " <<
  // nBad << endl;
  ...
  // Recover optimized Sim3
  ORB_SLAM3::VertexSim3Expmap* vSim3_recov =
      static_cast<ORB_SLAM3::VertexSim3Expmap*>(optimizer.vertex(0));
  g2oS12 = vSim3_recov->estimate();
...
void Optimizer::LocalInertialBA(KeyFrame* pKF, bool* pbStopFlag, Map* pMap,
  ...
  Verbose::PrintMess("LIBA bNonFixed?: " + to_string(bNonFixed),
                     Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("LIBA KFs visual outliers: " + to_string(vToErase.size()),
                     Verbose::VERBOSITY_DEBUG);
  ...
  /*double t_const = std::chrono::duration_cast<std::chrono::duration<double>
  >(t1 - t0).count(); double t_opt =
  std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1).count();
  double t_rec = std::chrono::duration_cast<std::chrono::duration<double> >(t3 -
  t2).count(); std::cout << " Construction time: " << t_const << std::endl;
...
Eigen::MatrixXd Optimizer::Condition(const Eigen::MatrixXd& H, const int& start,
                                     const int& end) {
...
Eigen::MatrixXd Optimizer::Sparsify(const Eigen::MatrixXd& H, const int& start1,
                                    const int& end1, const int& start2,
                                    const int& end2) {
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF,
  ...
  const int nExpectedSize =
      (vpWeldingKFs.size() + vpFixedKFs.size()) * vpMPs.size();
  ...
  for (unsigned int i = 0; i < vpMPs.size(); ++i) {
    ...
    for (map<KeyFrame*, tuple<int, int>>::const_iterator mit =
      ...
      KeyFrame* pKF = mit->first;
      if (spKeyFrameBA.find(pKF) == spKeyFrameBA.end() || pKF->isBad() ||
          pKF->mnId > maxKFid || pKF->mnBALocalForKF != pCurrentKF->mnId ||
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,
  ...
  // Verbose::PrintMess("LBA: There are " + to_string(vpMPs.size()) + " MPs to
  // optimize", Verbose::VERBOSITY_NORMAL);
  ...
  // Verbose::PrintMess("LBA: number total of edged -> " +
  // to_string(vpEdgeKFMono.size() + vpEdgeKFStereo.size()),
  // Verbose::VERBOSITY_NORMAL);

  map<int, int> mStatsObs;
  for (map<MapPoint*, int>::iterator it = mpObsMPs.begin();
       it != mpObsMPs.end(); ++it) {
    ...
    /*if(numObs < 5)
    {
        cout << "LBA: MP " << pMPi->mnId << " has " << numObs << " observations"
    << endl;
    }*/
  }

  /*for(map<int, int>::iterator it = mStatsObs.begin(); it != mStatsObs.end();
  ++it)
  {
      cout << "LBA: There are " << it->second << " MPs with " << it->first << "
  observations" << endl;
  }*/
  ...
  if (bDoMore) {
    ...
    Verbose::PrintMess("LBA: First optimization, there are " +
  ...
  Verbose::PrintMess("LBA: Second optimization, there are " +
                         to_string(badMonoMP) + " monocular and " +
                         to_string(badStereoMP) + " sterero bad edges",
                     Verbose::VERBOSITY_DEBUG);
  ...
  if (!vToErase.empty()) {
    ...
    Verbose::PrintMess("LBA: There are " + to_string(vToErase.size()) +
                           " observations whose will be deleted from the map",
                       Verbose::VERBOSITY_DEBUG);
    for (size_t i = 0; i < vToErase.size(); i++) {
      KeyFrame* pKFi = vToErase[i].first;
      MapPoint* pMPi = vToErase[i].second;
    ...
    Verbose::PrintMess("LBA: " + to_string(spErasedMPs.size()) +
                           " MPs had deleted observations",
                       Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "LBA: Current map is " + to_string(pMainKF->GetMap()->GetId()),
        Verbose::VERBOSITY_DEBUG);
    int numErasedMP = 0;
    for (MapPoint* pMPi : spErasedMPs) {
      if (pMPi->isBad()) {
        Verbose::PrintMess(
            "LBA: MP " + to_string(pMPi->mnId) +
                " has lost almost all the observations, its origin map is " +
                to_string(pMPi->mnOriginMapId),
            Verbose::VERBOSITY_DEBUG);
        numErasedMP++;
      }
    }
    Verbose::PrintMess(
        "LBA: " + to_string(numErasedMP) + " MPs had deleted from the map",
        Verbose::VERBOSITY_DEBUG);

    for (KeyFrame* pKFi : spErasedKFs) {
      int num_MPs = pKFi->GetMapPoints().size();
      int num_init_MPs = mpMPs_in_KF[pKFi];
      Verbose::PrintMess("LBA: Initially KF " + to_string(pKFi->mnId) +
                             " had " + to_string(num_init_MPs) +
                             ", at the end has " + to_string(num_MPs),
                         Verbose::VERBOSITY_DEBUG);
    }
  }
  for (unsigned int i = 0; i < vpMPs.size(); ++i) {
  ...
  for (KeyFrame* pKFi : vpAdjustKF) {
    ...
    if (numMonoOptPoints + numStereoOptPoints < 50) {
      Verbose::PrintMess("LBA ERROR: KF " + to_string(pKFi->mnId) +
                             " has only " + to_string(numMonoOptPoints) +
                             " monocular and " + to_string(numStereoOptPoints) +
                             " stereo points",
                         Verbose::VERBOSITY_DEBUG);
    }
    if (dist > 1.0) {
      if (bShowImages) {
        ...
        for (size_t i = 0; i < vpMonoMPsOpt.size(); ++i) {
        ...
        for (size_t i = 0; i < vpStereoMPsOpt.size(); ++i) {
        ...
        for (size_t i = 0; i < vpMonoMPsBad.size(); ++i) {
        ...
        for (size_t i = 0; i < vpStereoMPsBad.size(); ++i) {
        ...
        string namefile = "./test_LBA/LBA_KF" + to_string(pKFi->mnId) + "_" +
                          to_string(numPointsMono + numPointsStereo) + "_D" +
                          to_string(dist) + ".png";
        cv::imwrite(namefile, imLeft);

        Verbose::PrintMess("--LBA in KF " + to_string(pKFi->mnId),
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--Distance: " + to_string(dist) + " meters",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess(
            "--Number of observations: " + to_string(numMonoOptPoints) +
                " in mono and " + to_string(numStereoOptPoints) + " in stereo",
        ...
        Verbose::PrintMess("--Number of discarded observations: " +
                               ...
                               to_string(numStereoBadPoints) + " in stereo",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--To much distance correction in LBA: It has " +
                               to_string(mpObsKFs[pKFi]) + " observated MPs",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--To much distance correction in LBA: It has " +
                               to_string(mpObsFinalKFs[pKFi]) +
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF,
  ...
  for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame*, tuple<int, int>>::const_iterator
      ...
      if (pKFi->mnId > maxKFid) {
        Verbose::PrintMess("ID greater than current KF is",
                           Verbose::VERBOSITY_NORMAL);
        continue;
      }
  ...
  if (pbStopFlag)
    if (!*pbStopFlag) optimizer.optimize(5);
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame* pFrame,
  ...
  egr->setVertex(1, VG);
  cv::Mat cvInfoG =
      pFrame->mpImuPreintegrated->C.rowRange(9, 12).colRange(9, 12).inv(
          cv::DECOMP_SVD);
  Eigen::Matrix3d InfoG;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++) InfoG(r, c) = cvInfoG.at<float>(r, c);
  ...
  ear->setVertex(1, VA);
  cv::Mat cvInfoA =
      pFrame->mpImuPreintegrated->C.rowRange(12, 15).colRange(12, 15).inv(
          cv::DECOMP_SVD);
  Eigen::Matrix3d InfoA;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++) InfoA(r, c) = cvInfoA.at<float>(r, c);
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame* pFrame, bool bRecInit) {
  ...
  egr->setVertex(1, VG);
  cv::Mat cvInfoG =
      pFrame->mpImuPreintegratedFrame->C.rowRange(9, 12).colRange(9, 12).inv(
          cv::DECOMP_SVD);
  Eigen::Matrix3d InfoG;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++) InfoG(r, c) = cvInfoG.at<float>(r, c);
  ...
  ear->setVertex(1, VA);
  cv::Mat cvInfoA =
      pFrame->mpImuPreintegratedFrame->C.rowRange(12, 15).colRange(12, 15).inv(
          cv::DECOMP_SVD);
  Eigen::Matrix3d InfoA;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++) InfoA(r, c) = cvInfoA.at<float>(r, c);
