...
void KeyFrame::EraseMapPointMatch(MapPoint *pMP) {
  ...
  int leftIndex = static_cast<int>(get<0>(indexes));
  int rightIndex = static_cast<int>(get<1>(indexes));
...
void KeyFrame::SetBadFlag() {
...

  {
    ...
    while (!mspChildrens.empty()) {
      ...
      if (bContinue) {
        if (pC->mnId == pP->mnId) {
          /*cout << "ERROR: The parent and son can't be the same KF. ID: " <<
          pC->mnId << endl; cout << "Current KF: " << mnId << endl; cout <<
          "Parent of the map: " << endl;*/
        }
        pC->ChangeParent(pP);
    ...
    if (mpParent) {
      ...
      // cout << "Error: KF haven't got a parent, it is imposible reach this
      // code point without him" << endl;
    }
    mbBad = true;
  }
