...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  if (pPrevF) {
    if (!pPrevF->mVw.empty()) mVw = pPrevF->mVw.clone();
  } else {
...
void Frame::SetVelocity(const cv::Mat &Vwb) { mVw = Vwb.clone(); }

void Frame::SetImuPoseVelocity(const cv::Mat &Rwb, const cv::Mat &twb,
                               const cv::Mat &Vwb) {
  mVw = Vwb.clone();
  cv::Mat Rbw = Rwb.t();
  cv::Mat tbw = -Rbw * twb;
...
cv::Mat Frame::GetImuPose() {
  ...
  Twb.rowRange(0, 3).colRange(0, 3) =
      mRwc * mImuCalib.Tcb.rowRange(0, 3).colRange(0, 3);
...
cv::Mat Frame::inRefCoordinates(cv::Mat pCw) { return mRcw * pCw + mtcw; }
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
             ...
             ORBextractor *extractorRight, ORBVocabulary *voc, cv::Mat &K,
             cv::Mat &distCoef, const float &bf, const float &thDepth,
             GeometricCamera *pCamera, GeometricCamera *pCamera2, cv::Mat &Tlr,
