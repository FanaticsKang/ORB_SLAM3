...
cv::Mat TwoViewReconstruction::ComputeH21(const vector<cv::Point2f> &vP1,
...
cv::Mat TwoViewReconstruction::ComputeF21(const vector<cv::Point2f> &vP1,
                                          const vector<cv::Point2f> &vP2) {
  const int N = vP1.size();
...
float TwoViewReconstruction::CheckHomography(const cv::Mat &H21,
                                             const cv::Mat &H12,
...
bool TwoViewReconstruction::ReconstructF(
    vector<bool> &vbMatchesInliers, cv::Mat &F21, cv::Mat &K, cv::Mat &R21,
...
bool TwoViewReconstruction::ReconstructH(
    vector<bool> &vbMatchesInliers, cv::Mat &H21, cv::Mat &K, cv::Mat &R21,
    cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated,
    float minParallax, int minTriangulated) {
  int N = 0;
  for (size_t i = 0, iend = vbMatchesInliers.size(); i < iend; i++)
...
void TwoViewReconstruction::Triangulate(const cv::KeyPoint &kp1,
...
int TwoViewReconstruction::CheckRT(
    ...
    vector<bool> &vbMatchesInliers, const cv::Mat &K, vector<cv::Point3f> &vP3D,
...
void TwoViewReconstruction::DecomposeE(const cv::Mat &E, cv::Mat &R1,
                                       cv::Mat &R2, cv::Mat &t) {
  cv::Mat u, w, vt;
