...
ImuCamPose::ImuCamPose(KeyFrame* pKF) : its(0) {
  ...
  Rcb[0] =
      Converter::toMatrix3d(pKF->mImuCalib.Tcb.rowRange(0, 3).colRange(0, 3));
...
ImuCamPose::ImuCamPose(Frame* pF) : its(0) {
  ...
  Rcb[0] =
      Converter::toMatrix3d(pF->mImuCalib.Tcb.rowRange(0, 3).colRange(0, 3));
  Rbc[0] = Rcb[0].transpose();
...
void EdgeStereo::linearizeOplus() {
  ...
  /*const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[1]);
  const g2o::VertexSBAPointXYZ* VPoint = static_cast<const
...
/*Eigen::Vector2d EdgeMonoInvdepth::cam_project(const Eigen::Vector3d &
trans_xyz) const{ double invZ = 1./trans_xyz[2]; Eigen::Vector2d res; res[0] =
invZ*trans_xyz[0]*fx + cx; res[1] = invZ*trans_xyz[1]*fy + cy; return res;
}

Eigen::Vector3d EdgeMonoInvdepth::cam_unproject(const double u, const double v,
const double invDepth) const{ Eigen::Vector3d res; res[2] = 1./invDepth; double
z_x=res[2]/fx; double z_y=res[2]/fy; res[0] = (u-cx)*z_x; res[1] = (v-cy)*z_y;
...
void EdgeMonoInvdepth::linearizeOplus()
{
    ...
    g2o::VertexSE3Expmap *vHost =
static_cast<g2o::VertexSE3Expmap*>(_vertices[1]); g2o::VertexSE3Expmap *vObs =
static_cast<g2o::VertexSE3Expmap*>(_vertices[2]);
    ...
    Eigen::Vector3d o_X_j =
cam_unproject(vPt->estimate().u,vPt->estimate().v,o_rho_j); Eigen::Vector3d
i_X_j = Ti0.map(o_X_j); double i_rho_j = 1./i_X_j[2]; Eigen::Vector2d i_proj_j =
cam_project(i_X_j);
...
Eigen::Vector2d EdgeMonoInvdepthBody::cam_project(const Eigen::Vector3d &
trans_xyz) const{ double invZ = 1./trans_xyz[2]; Eigen::Vector2d res; res[0] =
invZ*trans_xyz[0]*fx + cx; res[1] = invZ*trans_xyz[1]*fy + cy; return res;
}

Eigen::Vector3d EdgeMonoInvdepthBody::cam_unproject(const double u, const double
v, const double invDepth) const{ Eigen::Vector3d res; res[2] = 1./invDepth;
  double z_x=res[2]/fx;
  double z_y=res[2]/fy;
  res[0] = (u-cx)*z_x;
  res[1] = (v-cy)*z_y;
...
EdgeInertial::EdgeInertial(IMU::Preintegrated* pInt)
  ...
  Matrix9d Info;
  for (int r = 0; r < 9; r++)
    for (int c = 0; c < 9; c++) Info(r, c) = cvInfo.at<float>(r, c);
...
void EdgeInertial::linearizeOplus() {
  ...
  _jacobianOplus[0].block<3, 3>(3, 0) =
      Skew(Rbw1 * (VV2->estimate() - VV1->estimate() - g * dt));  // OK
  _jacobianOplus[0].block<3, 3>(6, 0) =
      Skew(Rbw1 * (VP2->estimate().twb - VP1->estimate().twb -
...
void EdgeInertialGS::linearizeOplus() {
  ...
  _jacobianOplus[0].block<3, 3>(3, 0) =
      Skew(Rbw1 * (s * (VV2->estimate() - VV1->estimate()) - g * dt));
  _jacobianOplus[0].block<3, 3>(6, 0) = Skew(
      Rbw1 *
...
Eigen::Matrix3d ExpSO3(const double x, const double y, const double z) {
  ...
  if (d < 1e-5) {
    ...
    return Converter::toMatrix3d(
        IMU::NormalizeRotation(Converter::toCvMat(res)));
  } else {
