...
Eigen::Matrix<double, 3, 3> ExpSO3(const double &x, const double &y,
                                   const double &z) {
...
cv::Mat LogSO3(const cv::Mat &R) {
  ...
  cv::Mat w =
      (cv::Mat_<float>(3, 1) << (R.at<float>(2, 1) - R.at<float>(1, 2)) / 2,
...
cv::Mat InverseRightJacobianSO3(const float &x, const float &y,
                                const float &z) {
  cv::Mat I = cv::Mat::eye(3, 3, CV_32F);
  const float d2 = x * x + y * y + z * z;
  const float d = sqrt(d2);
...
cv::Mat InverseRightJacobianSO3(const cv::Mat &v) {
  return InverseRightJacobianSO3(v.at<float>(0), v.at<float>(1),
                                 v.at<float>(2));
}

IntegratedRotation::IntegratedRotation(const cv::Point3f &angVel,
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration,
  ...
  cv::Mat acc = (cv::Mat_<float>(3, 1) << acceleration.x - b.bax,
                 acceleration.y - b.bay, acceleration.z - b.baz);
  cv::Mat accW = (cv::Mat_<float>(3, 1) << angVel.x - b.bwx, angVel.y - b.bwy,
                  angVel.z - b.bwz);
  ...
  cv::Mat Wacc = (cv::Mat_<float>(3, 3) << 0, -acc.at<float>(2),
                  acc.at<float>(1), acc.at<float>(2), 0, -acc.at<float>(0),
  ...
  // Update covariance
  C.rowRange(0, 9).colRange(0, 9) =
      A * C.rowRange(0, 9).colRange(0, 9) * A.t() + B * Nga * B.t();
  C.rowRange(9, 15).colRange(9, 15) =
      C.rowRange(9, 15).colRange(9, 15) + NgaWalk;
...
cv::Mat Preintegrated::GetDeltaRotation(const Bias &b_) {
  std::unique_lock<std::mutex> lock(mMutex);
  cv::Mat dbg =
      (cv::Mat_<float>(3, 1) << b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz);
...
cv::Mat Preintegrated::GetDeltaVelocity(const Bias &b_) {
  ...
  cv::Mat dbg =
      (cv::Mat_<float>(3, 1) << b_.bwx - b.bwx, b_.bwy - b.bwy, b_.bwz - b.bwz);
  cv::Mat dba =
      (cv::Mat_<float>(3, 1) << b_.bax - b.bax, b_.bay - b.bay, b_.baz - b.baz);
...
Eigen::Matrix<double, 15, 15> Preintegrated::GetInformationMatrix() {
  ...
  if (Info.empty()) {
    ...
    Info.rowRange(0, 9).colRange(0, 9) =
        C.rowRange(0, 9).colRange(0, 9).inv(cv::DECOMP_SVD);
    for (int i = 9; i < 15; i++)
  ...
  for (int i = 0; i < 15; i++)
    for (int j = 0; j < 15; j++) EI(i, j) = Info.at<float>(i, j);
  return EI;
}
...
void Calib::Set(const cv::Mat &Tbc_, const float &ng, const float &na,
  ...
  Tcb.rowRange(0, 3).col(3) =
      -Tbc.rowRange(0, 3).colRange(0, 3).t() * Tbc.rowRange(0, 3).col(3);
