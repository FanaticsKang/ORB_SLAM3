...
cv::Mat FrameDrawer::DrawFrame(bool bOldFeatures) {
  ...
  } else if (state == Tracking::OK && bOldFeatures)  // TRACKING
  {
    ...
    float u, v;
    bool bIsInImage = currentFrame.ProjectPointDistort(vpMatchedMPs[i] ,
    point3d_proy, u, v); if(bIsInImage)
    {
        //cout << "-Point is out of the image" << point3d_proy.x << ", " <<
    point3d_proy.y << endl;
    ...
    //                    mnTracked++;
    //                }
    //                else // This is match to a "visual odometry" MapPoint
    //                created in the last frame
    //                {
    //                    cv::rectangle(im,pt1,pt2,cv::Scalar(255,0,0));
    //                    cv::circle(im,vCurrentKeys[i].pt,2,cv::Scalar(255,0,0),-1);
    //                    mnTrackedVO++;
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (pMP) {
        if (!pTracker->mCurrentFrame.mvbOutlier[i]) {
          ...
          // cv::Point2f point3d_proy;
          // float u, v;
          // bool bIsInImage = mCurrentFrame.ProjectPointDistort(pMP,
          // point3d_proy, u, v); if(bIsInImage)
          //{
          // mvMatchedKeys.push_back(mvCurrentKeys[i]);
