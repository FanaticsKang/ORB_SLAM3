...
void System::SaveTrajectoryEuRoC(const string &filename) {
  ...
  for (list<cv::Mat>::iterator lit = mpTracker->mlRelativeFramePoses.begin(),
    ...
    if (!pKF || pKF->GetMap() != pBiggerMap) {
      ...
      /*if(pKF)
          cout << "--Parent KF " << pKF->mnId << " is from another map " <<
         pKF->GetMap()->GetId() << endl;*/
      continue;
    }
...
/*void System::SaveAtlas(int type){
    cout << endl << "Enter the name of the file if you want to save the current
Atlas session. To exit press ENTER: "; string saveFileName;
...
CalculateCheckSum(mStrVocabularyFilePath,TEXT_FILE); std::size_t found =
mStrVocabularyFilePath.find_last_of("/\\"); string strVocabularyName =
mStrVocabularyFilePath.substr(found+1);
...
was created " << endl; cout << "-Vocabulary name: " << strFileVoc << endl;
...
string System::CalculateCheckSum(string filename, int type)
{
    ...
    if ( !f.is_open() )
    {
        cout << "[E] Unable to open the in file " << filename << " for Md5
hash." << endl; return checksum;
    }
