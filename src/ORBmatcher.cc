...
bool ORBmatcher::CheckDistEpipolarLine2(const cv::KeyPoint &kp1,
                                        const cv::KeyPoint &kp2,
                                        const cv::Mat &F12,
                                        const KeyFrame *pKF2, const float unc) {
  // Epipolar line in second image l = x1'F12 = [a b c]
  const float a = kp1.pt.x * F12.at<float>(0, 0) +
                  kp1.pt.y * F12.at<float>(1, 0) + F12.at<float>(2, 0);
  const float b = kp1.pt.x * F12.at<float>(0, 1) +
  ...
  while (KFit != KFend && Fit != Fend) {
    if (KFit->first == Fit->first) {
      ...
      for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
        const int realIdxKF = static_cast<int>(vIndicesKF[iKF]);
        ...
        for (size_t iF = 0; iF < vIndicesF.size(); iF++) {
          if (F.Nleft == -1) {
            ...
            const int realIdxF = static_cast<int>(vIndicesF[iF]);
        ...
        if (bestDist1 <= TH_LOW) {
          if (static_cast<float>(bestDist1) <
            ...
            if (mbCheckOrientation) {
              cv::KeyPoint &Fkp = (!pKF->mpCamera2 || F.Nleft == -1)
                                      ? F.mvKeys[bestIdxF]
                                      : (bestIdxF >= F.Nleft)
                                            ? F.mvKeysRight[bestIdxF - F.Nleft]
                                            : F.mvKeys[bestIdxF];
...
int ORBmatcher::SearchForTriangulation(
  ...
  if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
    ...
    tll = pKF1->GetRotation() *
              (-pKF2->GetRotation().t() * pKF2->GetTranslation()) +
          pKF1->GetTranslation();
    tlr = pKF1->GetRotation() *
              (-pKF2->GetRightRotation().t() * pKF2->GetRightTranslation()) +
          pKF1->GetTranslation();
    trl = pKF1->GetRightRotation() *
              (-pKF2->GetRotation().t() * pKF2->GetTranslation()) +
          pKF1->GetRightTranslation();
    trr = pKF1->GetRightRotation() *
              (-pKF2->GetRightRotation().t() * pKF2->GetRightTranslation()) +
          pKF1->GetRightTranslation();
  }
    ...
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          if (pCamera1->epipolarConstrain(pCamera2, kp1, kp2, R12, t12,
                                          pKF1->mvLevelSigma2[kp1.octave],
                                          pKF2->mvLevelSigma2[kp2.octave]) ||
              bCoarse)  // MODIFICATION_2
          {
            bestIdx2 = idx2;
            bestDist = dist;
          }
    ...
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          cv::Mat x3D;
          if (pCamera1->matchAndtriangulate(kp1, kp2, pCamera2, Tcw1, Tcw2,
                                            pKF1->mvLevelSigma2[kp1.octave],
                                            pKF2->mvLevelSigma2[kp2.octave],
                                            x3D)) {
            bestIdx2 = idx2;
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints,
  ...
  for (int i = 0; i < nMPs; i++) {
    ...
    const vector<size_t> vIndices =
        pKF->GetFeaturesInArea(uv.x, uv.y, radius, bRight);
  ...
  for (int i = 0; i < LastFrame.N; i++) {
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        ...
        if (uv.x < CurrentFrame.mnMinX || uv.x > CurrentFrame.mnMaxX) continue;
        if (uv.y < CurrentFrame.mnMinY || uv.y > CurrentFrame.mnMaxY) continue;
        ...
        if (bForward)
          vIndices2 =
              CurrentFrame.GetFeaturesInArea(uv.x, uv.y, radius, nLastOctave);
        else if (bBackward)
          vIndices2 = CurrentFrame.GetFeaturesInArea(uv.x, uv.y, radius, 0,
                                                     nLastOctave);
        else
          vIndices2 = CurrentFrame.GetFeaturesInArea(
              uv.x, uv.y, radius, nLastOctave - 1, nLastOctave + 1);
        ...
        if (CurrentFrame.Nleft != -1) {
          ...
          if (bForward)
            ...
            vIndices2 = CurrentFrame.GetFeaturesInArea(
                uv.x, uv.y, radius, nLastOctave - 1, nLastOctave + 1, true);
    ...
    if (pMP) {
      if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
        ...
        const vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(
            uv.x, uv.y, radius, nPredictedLevel - 1, nPredictedLevel + 1);
