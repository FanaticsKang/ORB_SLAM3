...
bool Initializer::Initialize(const Frame &CurrentFrame,
  ...
  if (RH > 0.40)  // if(RH>0.40)
  {
    ...
    return ReconstructH(vbMatchesInliersH, H, K, R21, t21, vP3D, vbTriangulated,
                        minParallax, 50);
  } else  // if(pF_HF>0.6)
  {
    // cout << "Initialization from Fundamental" << endl;
    return ReconstructF(vbMatchesInliersF, F, K, R21, t21, vP3D, vbTriangulated,
                        minParallax, 50);
  }
...
int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t,
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
    ...
    float squareError1 = (uv1.x - kp1.pt.x) * (uv1.x - kp1.pt.x) +
    ...
    float squareError2 = (uv2.x - kp2.pt.x) * (uv2.x - kp2.pt.x) +
